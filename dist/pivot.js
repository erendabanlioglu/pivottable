...
*   --------
*   cellLimit
*   unusedAttrsVertical set always false
*   showZero: true line 71
*   Double Sum Percentage feature added
*   Heatmap double table added
*/
...
(function() {
  ...
  callWithJQuery(function($) {
    ...
    renderers = {
      ...
      "Double Heatmap": function(pvtData, opts) {
        return $(pivotTableRenderer(pvtData, opts)).heatmap("doubleheatmap");
      }
    };
    locales = {
      en: {
        ...
        localeStrings: {
          ...
          tooManyCells: "Too many cells for display. Try other combination.",
...
/* DOUBLE SUM FEATURE START */
        if(totalAggregator.name=='doubleSum' && totalAggregator.numInputs==0 )
        {
                ...
                if(!doubleSumInit)
                {
                    ...
                    for(numItem=1 ; numItem <= totalAggregator.value().length ; numItem++)
                    {
                        th = document.createElement("th");
                        th.className = "pvtTotalLabel";
                        th.innerHTML = attributes[1].attr2;

                        if(numItem==totalAggregator.value().length)
                 ...
                 for(numItem=0 ; numItem <= totalAggregator.value().length ; numItem++)
                {
                    td = document.createElement("td");
                    var varName = [numItem == '0' ? 'sum' : 'sum2'];
                    //varName = [numItem == '3' ? 'sum' : 'sum2'];

                    td.className = "pvtTotal rowTotal "+varName;
                    td.innerHTML = totalAggregator.format(totalAggregator[varName]);
                    td.setAttribute("data-value", totalAggregator[varName]);

                    if (numItem == totalAggregator.value().length) {
                        td.className = "pvtTotal rowTotal percentage";

                        var percentage;
                        if(totalAggregator.sum>totalAggregator.sum2)
                          percentage = ((totalAggregator.sum2) / (totalAggregator.sum))*100 ;
                        else
                          percentage = ((totalAggregator.sum) / (totalAggregator.sum2))*100 ;

                         if(percentage==null || percentage==undefined || totalAggregator.sum<0 ||totalAggregator.sum2<0)
                          percentage = 0;

                        td.innerHTML =   totalAggregator.format(percentage)+"%";
                        td.setAttribute("data-value", percentage);
                    }

                    td.setAttribute("data-for", "row" + i+numItem);
                    tr.appendChild(td);
                    result.appendChild(tr);
                }
        }
...
/* DOUBLE SUM FEATURE START*/  /* Column grand sums and Average percentage */
      if(totalAggregator.name=="doubleSum" && totalAggregator.numInputs==0 )
      {
            for(numItem=0 ; numItem <= totalAggregator.value().length ; numItem++)
            {
      ...
      switch (scope) {
        ...
        case "colheatmap":
          for (j = n = 0, ref1 = numCols; 0 <= ref1 ? n < ref1 : n > ref1; j = 0 <= ref1 ? ++n : --n) {
            ...
            heatmapper(".pvtTotal.colTotal", "red");
         case "doubleheatmap":
            heatmapper(".pvtTotal.rowTotal.sum", "red");
            heatmapper(".pvtTotal.rowTotal.sum2", "green");
            heatmapper(".pvtTotal.rowTotal.percentage", "green");
          break;
      }
      return this;
    };
